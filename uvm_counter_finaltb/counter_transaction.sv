//=============================================================================
// counter_transaction.sv  (unchanged from Lab11)
//
...
// by the monitor so that the scoreboard can compare expected vs actual.
//
...
//   act_ovf     – overflow flag observed at end of operation
//   act_unf     – underflow flag observed at end of operation
//
...
class counter_transaction extends uvm_sequence_item;
  ...
  //--------------------------------------------------------------------------
  // Stimulus fields — driven by the sequence, used by the driver
  //--------------------------------------------------------------------------
  ...
  // These are NOT randomised; they are filled in by counter_monitor.
  //--------------------------------------------------------------------------
  ...
  constraint valid {
    ...
    up_cycles   <= 10; down_cycles <= 10;  // limit multi-cycle count length
    ...
    if (test_type == TEST_COUNT_UP)   { up_cycles >= 1; down_cycles == 0; start_count >= 1; }
    // For COUNT_DOWN: must count at least one cycle downward
    if (test_type == TEST_COUNT_DOWN) { down_cycles >= 1; up_cycles == 0; start_count >= 1; }
  ...
  // transaction, used in UVM_INFO prints and debug logs.
  virtual function string convert2string();
    return $sformatf("test_type=%s load=%0d start=%0d up=%0d dn=%0d | act: count=%0d ovf=%0b unf=%0b",
                     test_type.name(), load_value, start_count,
                     up_cycles, down_cycles,
                     act_count, act_ovf, act_unf);
  endfunction
